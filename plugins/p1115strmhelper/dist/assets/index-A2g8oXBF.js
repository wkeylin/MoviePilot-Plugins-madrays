...
const {computed: computed$1D,nextTick: nextTick$q,onBeforeMount: onBeforeMount$4,onBeforeUnmount: onBeforeUnmount$d,ref: ref$P,shallowRef: shallowRef$M,toRef: toRef$$,vShow,watch: watch$F,withDirectives: withDirectives$1} = await importShared('vue');
...
const VImg = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const currentSrc = shallowRef$M(''); // Set from srcset
    const image = ref$P();
    const state = shallowRef$M(props.eager ? 'loading' : 'idle');
    const naturalWidth = shallowRef$M();
    const naturalHeight = shallowRef$M();
    ...
    const isBooted = shallowRef$M(false);
    {
...
const {computed: computed$1B,shallowRef: shallowRef$L} = await importShared('vue');
...
const VToolbar = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isExtended = shallowRef$L(!!(props.extended || slots.extension?.()));
...
// Utilities
const {computed: computed$1A,onBeforeUnmount: onBeforeUnmount$c,onMounted: onMounted$f,ref: ref$O,shallowRef: shallowRef$K,watch: watch$E} = await importShared('vue');
// Composables
...
function useScroll(props) {
  ...
  const currentScroll = shallowRef$K(0);
  const savedScroll = shallowRef$K(0);
  const currentThreshold = shallowRef$K(0);
  const isScrollActive = shallowRef$K(false);
  const isScrollingUp = shallowRef$K(false);
...
// Utilities
const {onMounted: onMounted$e,readonly: readonly$2,shallowRef: shallowRef$J,toRef: toRef$Z} = await importShared('vue');


// Composables
function useSsrBoot() {
  const isBooted = shallowRef$J(false);
  onMounted$e(() => {
...
const {computed: computed$1z,ref: ref$N,shallowRef: shallowRef$I,toRef: toRef$Y,watchEffect: watchEffect$i} = await importShared('vue');
...
const VAppBar = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const {
      ...
      layoutSize: height,
      elementSize: shallowRef$I(undefined),
      active: isActive,
...
const {computed: computed$1y,inject: inject$l,onBeforeUnmount: onBeforeUnmount$b,onMounted: onMounted$d,onUpdated,provide: provide$g,reactive: reactive$2,toRef: toRef$U,unref: unref$2,useId: useId$d,watch: watch$D} = await importShared('vue');
...
// Utilities
const {onBeforeUnmount: onBeforeUnmount$a,ref: ref$M,shallowRef: shallowRef$G,watch: watch$C} = await importShared('vue');
function useIntersectionObserver(callback, options) {
  const intersectionRef = ref$M();
...
const {ref: ref$L,toRef: toRef$T,watchEffect: watchEffect$h} = await importShared('vue');
...
const {computed: computed$1t,inject: inject$k,nextTick: nextTick$n,ref: ref$K,shallowRef: shallowRef$F,toRef: toRef$M,useId: useId$b} = await importShared('vue');
...
const {computed: computed$1r,inject: inject$j,markRaw,provide: provide$e,ref: ref$J,shallowRef: shallowRef$E,toRef: toRef$J,watch: watch$A} = await importShared('vue');
...
// Utilities
const {computed: computed$1q,nextTick: nextTick$m,onBeforeMount: onBeforeMount$3,onBeforeUnmount: onBeforeUnmount$9,onMounted: onMounted$c,ref: ref$I,shallowRef: shallowRef$D,unref: unref$1,useId: useId$a,watch: watch$z} = await importShared('vue');
...
function useValidation(props) {
  ...
  const internalErrorMessages = ref$I([]);
  const isPristine = shallowRef$D(true);
...
const {computed: computed$1o,shallowRef: shallowRef$C,watch: watch$y} = await importShared('vue');
...
const VSlideGroup = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isOverflowing = shallowRef$C(false);
    const scrollOffset = shallowRef$C(0);
    const containerSize = shallowRef$C(0);
    const contentSize = shallowRef$C(0);
...
// Utilities
const {computed: computed$1m,inject: inject$i,provide: provide$d,shallowRef: shallowRef$B} = await importShared('vue');

// List
const ListKey = Symbol.for('vuetify:list');
function createList() {
  const parent = inject$i(ListKey, {
    hasPrepend: shallowRef$B(false),
...
const {computed: computed$1l,inject: inject$h,onBeforeMount: onBeforeMount$2,onBeforeUnmount: onBeforeUnmount$8,provide: provide$c,ref: ref$H,shallowRef: shallowRef$A,toRaw: toRaw$2,toRef: toRef$F,toValue: toValue$1} = await importShared('vue');
...
const emptyNested = {
  ...
  root: {
    register: () => null,
    unregister: () => null,
    parents: ref$H(new Map()),
    children: ref$H(new Map()),
    open: () => null,
    openOnSelect: () => null,
    activate: () => null,
    select: () => null,
    activatable: ref$H(false),
    selectable: ref$H(false),
    opened: ref$H(new Set()),
    activated: ref$H(new Set()),
    selected: ref$H(new Map()),
    selectedValues: ref$H([]),
    getPath: () => []
  }
};
...
const useNested = props => {
  let isUnmounted = false;
  const children = ref$H(new Map());
  const parents = ref$H(new Map());
...
// Utilities
const {computed: computed$1h,shallowRef: shallowRef$z,watchEffect: watchEffect$g} = await importShared('vue');
// Composables
...
function useItems(props) {
  ...
  const itemsMap = shallowRef$z(new Map());
  const keylessItems = shallowRef$z([]);
  watchEffect$g(() => {
...
const {computed: computed$1g,ref: ref$G,shallowRef: shallowRef$y,toRef: toRef$D} = await importShared('vue');
...
const {computed: computed$1f,nextTick: nextTick$l,onScopeDispose: onScopeDispose$8,ref: ref$F,watch: watch$w} = await importShared('vue');
...
function useLocationStrategies(props, data) {
  const contentStyles = ref$F({});
  const updateLocation = ref$F();
  if (IN_BROWSER) {
...
const {computed: computed$1e,effectScope,inject: inject$g,mergeProps: mergeProps$b,nextTick: nextTick$k,onScopeDispose: onScopeDispose$6,ref: ref$E,watch: watch$v,watchEffect: watchEffect$e} = await importShared('vue');
...
const {onMounted: onMounted$b,shallowRef: shallowRef$x} = await importShared('vue');
function useHydration() {
  if (!IN_BROWSER) return shallowRef$x(false);
  const {
    ssr
  } = useDisplay();
  if (ssr) {
    const isMounted = shallowRef$x(false);
    onMounted$b(() => {
      isMounted.value = true;
    });
    return isMounted;
  } else {
    return shallowRef$x(true);
  }
}

// Utilities
const {shallowRef: shallowRef$w,toRef: toRef$C,watch: watch$u} = await importShared('vue');
const makeLazyProps = propsFactory({
  eager: Boolean
}, 'lazy');
function useLazy(props, active) {
  const isBooted = shallowRef$w(false);
...
const {inject: inject$f,onScopeDispose: onScopeDispose$5,provide: provide$b,reactive,readonly: readonly$1,shallowRef: shallowRef$v,toRaw: toRaw$1,toRef: toRef$B,toValue,watchEffect: watchEffect$d} = await importShared('vue');
...
function useStack(isActive, zIndex, disableGlobalStack) {
  ...
  const _zIndex = shallowRef$v(Number(toValue(zIndex)));
  ...
  const globalTop = shallowRef$v(true);
  if (createStackEntry) {
    watchEffect$d(() => {
...
const {computed: computed$1c,mergeProps: mergeProps$a,onBeforeUnmount: onBeforeUnmount$7,ref: ref$D,Teleport,Transition: Transition$1,watch: watch$t} = await importShared('vue');
...
const {computed: computed$1b,inject: inject$e,mergeProps: mergeProps$9,nextTick: nextTick$j,onBeforeUnmount: onBeforeUnmount$6,onDeactivated,provide: provide$a,ref: ref$C,shallowRef: shallowRef$u,toRef: toRef$A,useId: useId$7,watch: watch$s} = await importShared('vue');
...
const VMenu = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const openChildren = shallowRef$u(new Set());
...
const {computed: computed$1a,ref: ref$B,toRef: toRef$y,useId: useId$6,watch: watch$r} = await importShared('vue');
...
const VField = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const labelRef = ref$B();
    const floatingLabelRef = ref$B();
    const controlRef = ref$B();
...
const {cloneVNode,computed: computed$19,nextTick: nextTick$i,ref: ref$A} = await importShared('vue');
...
const {computed: computed$18,nextTick: nextTick$h,onScopeDispose: onScopeDispose$4,ref: ref$z,shallowRef: shallowRef$t,watch: watch$p,watchEffect: watchEffect$c} = await importShared('vue');
const UP = -1;
const DOWN = 1;
...
function useVirtual(props, items) {
  ...
  const itemHeight = shallowRef$t(0);
  watchEffect$c(() => {
    itemHeight.value = parseFloat(props.itemHeight || 0);
  });
  const first = shallowRef$t(0);
  const last = shallowRef$t(Math.ceil(
  // Assume 16px items filling the entire screen height if
  // not provided. This is probably incorrect but it minimises
  // the chance of ending up with empty space at the bottom.
  // The default value is set here to avoid poisoning getSize()
  (parseInt(props.height) || display.height.value) / (itemHeight.value || 16)) || 1);
  const paddingTop = shallowRef$t(0);
  const paddingBottom = shallowRef$t(0);

  /** The scrollable element */
  const containerRef = ref$z();
  /** An element marking the top of the scrollable area,
   * used to add an offset if there's padding or other elements above the virtual list */
  const markerRef = ref$z();
  ...
  const updateTime = shallowRef$t(0);
...
const {onMounted: onMounted$a,onScopeDispose: onScopeDispose$3,toRef: toRef$x} = await importShared('vue');
...
// Utilities
const {shallowRef: shallowRef$s,watch: watch$o} = await importShared('vue');


// Types

function useScrolling(listRef, textFieldRef) {
  const isScrolling = shallowRef$s(false);
  let scrollTimeout;
...
const {computed: computed$17,mergeProps: mergeProps$8,nextTick: nextTick$g,ref: ref$y,shallowRef: shallowRef$r,toRef: toRef$w,watch: watch$n} = await importShared('vue');
...
const VSelect = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isFocused = shallowRef$r(false);
...
// Utilities
const {computed: computed$16,shallowRef: shallowRef$q,unref,watchEffect: watchEffect$b,createVNode:_createVNode$1z,Fragment:_Fragment$t} = await importShared('vue');
/**
...
function useFilter(props, items, query, options) {
  const filteredItems = shallowRef$q([]);
  const filteredMatches = shallowRef$q(new Map());
...
const {computed: computed$15,mergeProps: mergeProps$7,nextTick: nextTick$f,ref: ref$x,shallowRef: shallowRef$p,watch: watch$m} = await importShared('vue');
...
const VAutocomplete = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isPristine = shallowRef$p(true);
    const listHasFocus = shallowRef$p(false);
    const vMenuRef = ref$x();
    const vVirtualScrollRef = ref$x();
    const selectionIndex = shallowRef$p(-1);
    const {
      items,
      transformIn,
      transformOut
    ...
    const isSelecting = shallowRef$p(false);
...
const {mergeProps: mergeProps$6,nextTick: nextTick$e,onBeforeUnmount: onBeforeUnmount$5,ref: ref$w,watch: watch$l} = await importShared('vue');
...
const {computed: computed$11,provide: provide$9,ref: ref$v,shallowRef: shallowRef$o,toRef: toRef$s,watch: watch$k} = await importShared('vue');
...
const VWindow = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isReversed = shallowRef$o(false);
    ...
    const transitionCount = shallowRef$o(0);
    const transitionHeight = ref$v(undefined);
...
const {onMounted: onMounted$9,ref: ref$u,watch: watch$j} = await importShared('vue');
...
const {computed: computed$10,inject: inject$d,nextTick: nextTick$d,shallowRef: shallowRef$n} = await importShared('vue');
...
const VWindowItem = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const isTransitioning = shallowRef$n(false);
...
const {createVNode:_createVNode$1f} = await importShared('vue');
const {computed: computed$$,onMounted: onMounted$8,ref: ref$t,shallowRef: shallowRef$m,watch: watch$i} = await importShared('vue');
...
const VColorPickerCanvas = defineComponent$1({
  ...
  setup(props, _ref) {
    ...
    const isInteracting = shallowRef$m(false);
    const canvasRef = ref$t();
    const canvasWidth = shallowRef$m(parseFloat(props.width));
    const canvasHeight = shallowRef$m(parseFloat(props.height));
    const _dotPosition = ref$t({
      x: 0,
      y: 0
    });
...
const {computed: computed$Z,nextTick: nextTick$c,provide: provide$8,ref: ref$s,shallowRef: shallowRef$l,toRef: toRef$r} = await importShared('vue');
...
const useSlider = _ref => {
  ...
  const mousePressed = shallowRef$l(false);
  const startOffset = shallowRef$l(0);
  const trackContainerRef = ref$s();
  const activeThumbRef = ref$s();
...
const {computed: computed$W,ref: ref$r} = await importShared('vue');
...
const {computed: computed$V,onBeforeMount,ref: ref$q,watch: watch$h} = await importShared('vue');
...
const {computed: computed$U,mergeProps: mergeProps$5,nextTick: nextTick$b,ref: ref$p,shallowRef: shallowRef$k,toRef: toRef$q,watch: watch$g} = await importShared('vue');
...
const VCombobox = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const _search = shallowRef$k(!props.multiple && !hasSelectionSlot.value ? model.value[0]?.title ?? '' : '');
...
const {computed: computed$T,ref: ref$o,toRaw,watchEffect: watchEffect$a} = await importShared('vue');
...
const {computed: computed$S,inject: inject$9,provide: provide$6,ref: ref$n} = await importShared('vue');
...
const {computed: computed$Q,inject: inject$7,provide: provide$4,shallowRef: shallowRef$j,toRef: toRef$o} = await importShared('vue');
...
function provideSelection(props, _ref9) {
  ...
  const lastSelectedIndex = shallowRef$j(null);
...
// Utilities
const {onBeforeUpdate,ref: ref$m} = await importShared('vue');
...
const {computed: computed$M,nextTick: nextTick$a,shallowRef: shallowRef$i,toRef: toRef$l} = await importShared('vue');
...
const VPagination = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const maxButtons = shallowRef$i(-1);
...
// Utilities
const {capitalize: capitalize$2,inject: inject$5,provide: provide$2,ref: ref$l,watchEffect: watchEffect$9} = await importShared('vue');
...
function createHeaders(props, options) {
  const headers = ref$l([]);
  const columns = ref$l([]);
  const sortFunctions = ref$l({});
  const sortRawFunctions = ref$l({});
  const filterFunctions = ref$l({});
  watchEffect$9(() => {
...
const {computed: computed$G,shallowRef: shallowRef$h,toRef: toRef$j,toRefs: toRefs$2} = await importShared('vue');
...
const {computed: computed$A,ref: ref$k,shallowRef: shallowRef$g,toRef: toRef$h,watch: watch$d} = await importShared('vue');
...
const VDatePickerMonth = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const adapter = useDate();
    const rangeStart = shallowRef$g();
    const rangeStop = shallowRef$g();
    const isReverse = shallowRef$g(false);
...
const {computed: computed$y,nextTick: nextTick$9,onMounted: onMounted$7,watchEffect: watchEffect$7} = await importShared('vue');
// Types
...
const {computed: computed$x,ref: ref$j,shallowRef: shallowRef$f,toRef: toRef$g,watch: watch$c} = await importShared('vue');
// Types
...
const VDatePicker = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const month = ref$j(Number(props.month ?? adapter.getMonth(adapter.startOfMonth(internal.value))));
    const year = ref$j(Number(props.year ?? adapter.getYear(adapter.startOfYear(adapter.setMonth(internal.value, month.value)))));
...
const {computed: computed$u,ref: ref$i,shallowRef: shallowRef$e,toRef: toRef$c,watchEffect: watchEffect$6} = await importShared('vue');
...
const {computed: computed$t,nextTick: nextTick$8,ref: ref$h,shallowRef: shallowRef$d,toRef: toRef$b,watch: watch$b} = await importShared('vue');
...
const VFileInput = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const vInputRef = ref$h();
    const vFieldRef = ref$h();
...
const {computed: computed$s,ref: ref$g,shallowRef: shallowRef$c,toRef: toRef$a,watchEffect: watchEffect$5} = await importShared('vue');
...
const {ref: ref$f} = await importShared('vue');
...
const {computed: computed$r,nextTick: nextTick$7,onMounted: onMounted$6,ref: ref$e,shallowRef: shallowRef$b,watch: watch$a} = await importShared('vue');
...
// Utilities
const {computed: computed$p,onBeforeUnmount: onBeforeUnmount$4,onMounted: onMounted$5,shallowRef: shallowRef$a,watch: watch$9} = await importShared('vue');
...
const {computed: computed$o,onBeforeUnmount: onBeforeUnmount$3,onMounted: onMounted$4,onScopeDispose: onScopeDispose$2,shallowRef: shallowRef$9,watchEffect: watchEffect$4} = await importShared('vue');
...
const {computed: computed$n,nextTick: nextTick$6,readonly,ref: ref$d,shallowRef: shallowRef$8,toRef: toRef$8,Transition,watch: watch$8} = await importShared('vue');
...
const {computed: computed$m,nextTick: nextTick$5,onMounted: onMounted$3,ref: ref$c,shallowRef: shallowRef$7,toRef: toRef$7,watch: watch$7,watchEffect: watchEffect$3} = await importShared('vue');
...
const {computed: computed$l,nextTick: nextTick$4,ref: ref$b,toRef: toRef$6,watch: watch$6} = await importShared('vue');
// Types
...
const VOtpInput = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const focusIndex = ref$b(-1);
    const contentRef = ref$b();
    const inputRef = ref$b([]);
...
const {createVNode:_createVNode$t} = await importShared('vue');
const {computed: computed$k,onBeforeUnmount: onBeforeUnmount$2,ref: ref$a,watch: watch$5,watchEffect: watchEffect$2} = await importShared('vue');
function floor(val) {
...
const {computed: computed$i,ref: ref$9} = await importShared('vue');
...
const VRangeSlider = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const startThumbRef = ref$9();
    const stopThumbRef = ref$9();
    const inputRef = ref$9();
    const {
      rtlClasses
    } = useRtl();
...
const {computed: computed$h,shallowRef: shallowRef$6,useId: useId$4} = await importShared('vue');
...
const {computed: computed$f,inject: inject$2,mergeProps: mergeProps$2,nextTick: nextTick$3,onMounted: onMounted$2,onScopeDispose,ref: ref$8,shallowRef: shallowRef$5,watch: watch$4,watchEffect: watchEffect$1} = await importShared('vue');
...
const {computed: computed$e,nextTick: nextTick$2,shallowRef: shallowRef$4,watch: watch$3} = await importShared('vue');
...
// Utilities
const {computed: computed$c,nextTick: nextTick$1,ref: ref$7,useId: useId$2,watch: watch$2,createVNode:_createVNode$k} = await importShared('vue');
...
const VTrendline = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const lastLength = ref$7(0);
    const path = ref$7(null);
...
const {computed: computed$a,ref: ref$6} = await importShared('vue');
...
const {ref: ref$5,toRef: toRef$5,useId: useId$1} = await importShared('vue');
...
const {createVNode:_createVNode$b} = await importShared('vue');
const {computed: computed$6,shallowRef: shallowRef$3,toRef: toRef$4} = await importShared('vue');
...
const {computed: computed$5,ref: ref$4} = await importShared('vue');
...
const {computed: computed$2,nextTick,onBeforeUnmount: onBeforeUnmount$1,onMounted: onMounted$1,ref: ref$3,shallowRef: shallowRef$2,watch: watch$1,watchEffect} = await importShared('vue');
...
const VTextarea = genericComponent()({
  ...
  setup(props, _ref) {
    ...
    const vInputRef = ref$3();
    const vFieldRef = ref$3();
    const controlHeight = shallowRef$2('');
    const textareaRef = ref$3();
    ...
    const sizerRef = ref$3();
    const rows = ref$3(Number(props.rows));
...
const {ref: ref$2,shallowRef: shallowRef$1,watch} = await importShared('vue');
// Types
...
const {computed,mergeProps: mergeProps$1,ref: ref$1,toRef,useId} = await importShared('vue');
  ...
  return (_openBlock(), _createBlock(_component_v_app, null, {
    default: _withCtx(() => [
      _createElementVNode("div", _hoisted_1, [
        ...
        }, null, 40, ["api", "onSwitch", "onClose", "onSave"]))
      ])
    ]),
    _: 1
  }))
