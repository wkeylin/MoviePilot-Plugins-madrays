  ...
  <div class="plugin-config">
    <v-card flat class="rounded border">
      ...
      <v-card-text class="px-3 py-2">
        <v-alert v-if="message.text" :type="message.type" density="compact" class="mb-2 text-caption" variant="tonal" closable>{{ message.text }}</v-alert>
        ...
        <v-row>
          <v-col cols="12" md="4">
                  <v-switch v-model="config.enabled" label="启用插件" color="primary" density="compact"></v-switch>
          </v-col>
          <v-col cols="12" md="8">
              <v-text-field 
                    v-model="config.cookies" 
                label="115 Cookie" 
                    hint="115网盘登录Cookie，可点击扫码图标获取"
                persistent-hint
                    density="compact"
                    variant="outlined"
                    hide-details="auto"
                    :append-icon="config.cookies ? 'mdi-check-circle' : 'mdi-qrcode-scan'"
                    @click:append="openQrCodeDialog"
              ></v-text-field>
          </v-col>
        </v-row>
        <v-row>
                <v-col cols="12" md="12">
                  <v-text-field 
                    ...
                    hint="用于生成STRM文件中的内网地址，如 http://192.168.1.100:3000"
                    persistent-hint
                    density="compact"
                    variant="outlined"
                    hide-details="auto"
        ...
        <v-row>
                <v-col cols="12" md="6">
                  <v-text-field 
                    v-model="config.user_rmt_mediaext" 
                    label="可整理媒体文件扩展名"
                ...
                <v-col cols="12" md="6">
                  <v-text-field 
                    v-model="config.user_download_mediaext" 
                    label="可下载媒体数据文件扩展名"
      ...
      <v-window v-model="activeTab">
        ...
        <v-window-item value="tab-transfer">
          <v-card-text>
            <v-row>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.transfer_monitor_enabled" label="整理事件监控" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.transfer_monitor_scrape_metadata_enabled" label="STRM自动刮削" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.transfer_monitor_media_server_refresh_enabled" label="媒体服务器刷新" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                <v-select
                        v-model="config.transfer_monitor_mediaservers"
                  label="媒体服务器"
                        :items="mediaservers"
                  multiple
                  chips
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        <div v-for="(pair, index) in transferPaths" :key="`transfer-${index}`" class="mb-2 d-flex align-center">
                          <div class="path-selector flex-grow-1 mr-2">
                            <v-text-field
                              ...
                              @click:append="openDirSelector(index, 'local', 'transfer')"
                            ></v-text-field>
                </div>
                          ...
                          <div class="path-selector flex-grow-1 ml-2">
                            <v-text-field
                              ...
                              @click:append="openDirSelector(index, 'remote', 'transfer')"
                            ></v-text-field>
                          </div>
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPath('transfer')">
                          添加路径
                        </v-btn>
                      </div>
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPath('mp')">
                          添加路径
                        </v-btn>
                      </div>
        ...
        <v-window-item value="tab-sync">
          <v-card-text>
            <v-row>
                    <v-col cols="12" md="4">
                      <v-switch v-model="config.timing_full_sync_strm" label="定期全量同步" color="primary"></v-switch>
              </v-col>
                    <v-col cols="12" md="4">
                      <v-text-field v-model="config.cron_full_sync_strm" label="运行全量同步周期" hint="Cron表达式" persistent-hint></v-text-field>
              </v-col>
                    <v-col cols="12" md="4">
                      <v-switch v-model="config.full_sync_auto_download_mediainfo_enabled" label="下载媒体数据文件" color="primary"></v-switch>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        <div v-for="(pair, index) in fullSyncPaths" :key="`full-${index}`" class="mb-2 d-flex align-center">
                          <div class="path-selector flex-grow-1 mr-2">
                            <v-text-field
                              v-model="pair.local"
                              label="本地STRM目录"
                              density="compact"
                              append-icon="mdi-folder"
                              @click:append="openDirSelector(index, 'local', 'fullSync')"
                            ></v-text-field>
                </div>
                          <v-icon>mdi-pound</v-icon>
                          <div class="path-selector flex-grow-1 ml-2">
                            <v-text-field
                              ...
                              density="compact"
                              append-icon="mdi-folder-network"
                              @click:append="openDirSelector(index, 'remote', 'fullSync')"
                            ></v-text-field>
                          </div>
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPath('fullSync')">
                          添加路径
                        </v-btn>
                      </div>
        ...
        <v-window-item value="tab-life">
          <v-card-text>
            <v-row>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.monitor_life_enabled" label="监控115生活事件" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.monitor_life_auto_download_mediainfo_enabled" label="下载媒体数据文件" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.monitor_life_scrape_metadata_enabled" label="STRM自动刮削" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.monitor_life_auto_remove_local_enabled" label="网盘删除本地同步删除" color="primary"></v-switch>
              </v-col>
            </v-row>
                  
            <v-row>
              <v-col cols="12" md="4">
                      <v-switch v-model="config.monitor_life_media_server_refresh_enabled" label="媒体服务器刷新" color="primary"></v-switch>
              </v-col>
                    <v-col cols="12" md="8">
                <v-select
                        v-model="config.monitor_life_mediaservers"
                  label="媒体服务器"
                        :items="mediaservers"
                  multiple
                  chips
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        <div v-for="(pair, index) in monitorLifePaths" :key="`life-${index}`" class="mb-2 d-flex align-center">
                          <div class="path-selector flex-grow-1 mr-2">
                            <v-text-field
                              ...
                              density="compact"
                              append-icon="mdi-folder"
                              @click:append="openDirSelector(index, 'local', 'monitorLife')"
                            ></v-text-field>
                </div>
                          <v-icon>mdi-pound</v-icon>
                          <div class="path-selector flex-grow-1 ml-2">
                            <v-text-field
                              ...
                              density="compact"
                              append-icon="mdi-folder-network"
                              @click:append="openDirSelector(index, 'remote', 'monitorLife')"
                            ></v-text-field>
                          </div>
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPath('monitorLife')">
                          添加路径
                        </v-btn>
                      </div>
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        <div v-for="(pair, index) in monitorLifeMpPaths" :key="`life-mp-${index}`" class="mb-2 d-flex align-center">
                          <div class="path-selector flex-grow-1 mr-2">
                            <v-text-field
                              v-model="pair.local"
                              label="媒体库服务器映射目录"
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPath('monitorLifeMp')">
                          添加路径
                        </v-btn>
                      </div>
          ...
          <v-card-text>
            ...
            <v-row>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.clear_recyclebin_enabled" label="清空回收站" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-switch v-model="config.clear_receive_path_enabled" label="清空我的接收目录" color="primary"></v-switch>
              </v-col>
              <v-col cols="12" md="3">
                      <v-text-field
              ...
              <v-col cols="12" md="3">
                      <v-text-field
                        v-model="config.cron_clear"
                        label="清理周期"
                        hint="清理任务执行周期，Cron表达式"
                        persistent-hint
                      ></v-text-field>
              </v-col>
            </v-row>
        ...
        <v-window-item value="tab-pan-transfer">
          <v-card-text>
            <v-row>
              <v-col cols="12" md="4">
                      <v-switch v-model="config.pan_transfer_enabled" label="网盘整理" color="primary"></v-switch>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col cols="12">
                      <div class="d-flex flex-column">
                        <div v-for="(path, index) in panTransferPaths" :key="`pan-${index}`" class="mb-2 d-flex align-center">
                          <v-text-field
                            v-model="path.path"
                    label="网盘待整理目录"
                            density="compact"
                        ...
                        <v-btn size="small" prepend-icon="mdi-plus" variant="outlined" class="mt-2 align-self-start" @click="addPanTransferPath">
                          添加路径
                        </v-btn>
                </div>
              </v-col>
          ...
          <!-- 操作按钮 -->
          <v-card-actions class="px-3 py-2 d-flex">
            <v-btn color="info" variant="text" @click="emit('switch')" size="small" prepend-icon="mdi-close">
              返回
            </v-btn>
          <v-spacer></v-spacer>
            <v-btn color="warning" variant="text" @click="triggerFullSync" :loading="syncLoading" size="small" prepend-icon="mdi-sync">
              全量同步
            </v-btn>
            <v-btn color="success" variant="text" @click="saveConfig" :loading="saveLoading" size="small" prepend-icon="mdi-content-save">
              保存配置
            </v-btn>
        </v-card-actions>
        </div>
    ...
    <v-dialog v-model="qrDialog.show" max-width="450">
      <v-card>
        ...
        <v-card-text class="text-center py-4">
          ...
          <div v-else-if="qrDialog.qrcode" class="d-flex flex-column align-center">
            ...
            <v-chip-group v-model="qrDialog.clientType" class="mb-3" mandatory selected-class="primary">
              <v-chip v-for="type in clientTypes" :key="type.value" 
                :value="type.value" variant="outlined" color="primary" size="small">
                {{ type.label }}
              </v-chip>
            </v-chip-group>
        ...
        <v-card-actions class="px-3 py-2">
          ...
          <v-spacer></v-spacer>
          <v-btn color="primary" variant="text" @click="refreshQrCode" :disabled="qrDialog.loading" size="small" prepend-icon="mdi-refresh">
            刷新二维码
          </v-btn>
        </v-card-actions>
      </v-card>
...
const checkQrCodeStatus = async () => {
  ...
  try {
    // 使用常量PLUGIN_ID
    const response = await props.api.get(`plugin/${PLUGIN_ID}/check_qrcode?uid=${qrDialog.uid}&client_type=${qrDialog.clientType}`);
...
/* 多彩标签页 */
:deep(.v-tabs) {
  background: linear-gradient(135deg, rgba(var(--v-theme-primary), 0.05), rgba(var(--v-theme-info), 0.05)) !important;
...
:deep(.v-switch--dirty .v-switch__thumb) {
...
:deep(.v-text-field:hover .v-field__outline__start),
:deep(.v-text-field:hover .v-field__outline__notch),
:deep(.v-text-field:hover .v-field__outline__end) {
  opacity: 0.8;
...
.section-title:before {
  ...
  height: 14px;
  width: 3px;
  background: linear-gradient(to bottom, rgba(var(--v-theme-primary), 1), rgba(var(--v-theme-info), 0.7));
  margin-right: 8px;
  border-radius: 2px;
}
</style> 
